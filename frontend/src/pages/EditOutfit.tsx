...
function EditOutfit() {
	...
	const fetchOutfit = async () => {
		try {
			...
			setSize(outfit.size as OutfitSize)
							...
							<div className="space-y-2">
								...
								<div className="grid grid-cols-2 gap-2">
									{(Object.keys(seasonLabels) as Season[]).map((season) => (
										<label
											key={season}
											className={`flex items-center gap-2 rounded-lg border p-3 cursor-pointer transition-colors ${seasons.includes(season)
							...
							<div className="space-y-2">
								...
								<div className="grid grid-cols-2 gap-2">
									{(Object.keys(propertyLabels) as OutfitProperty[]).map((property) => (
										<label
											key={property}
											className={`flex items-center gap-2 rounded-lg border p-3 cursor-pointer transition-colors ${properties.includes(property)
													? 'border-primary bg-primary/10'
													: 'border-border hover:border-primary/50'
												}`}
										>
